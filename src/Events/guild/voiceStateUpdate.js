...
module.exports = async(bot,oldState,newState) => {
    ...
    if(newState.member.roles.cache.some(role => role.id === "757589814643261470" || role.id === "757589866749100072")) return; //Если есть роли мута то выходим.
    ...
    const PrivatCategory = await bot.channels.cache.find(ch => ch.id === '757598794924949564');
        if(!newState.channel && oldState.channel.parentID === '757598794924949564' && oldState.channel.name != '📞 | Создать приват') {
        ...
        if (oldState.channel.parentID === '757598794924949564' && oldState.channel.name != '📞 | Создать приват') {
            if(oldState.channel.members.size <= 0) {
                return oldState.channel.delete({timeout:1000});
            }
        }
