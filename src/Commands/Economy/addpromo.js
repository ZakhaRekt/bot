...
module.exports = {
	name: "addpromo",
	category: "economy",
	description: "Добавление промокода на коины Администратором",
	...
	async run(bot, message, args) {
		...
		if (message.member.hasPermission('ADMINISTRATOR')) {
			...
			Promo.findOne({ name: args[0] }, async (err, data) => {
				if (!data) {
					let promo = new Promo({ name: args[0], amountUses: parseInt(args[1]), coinsPerUse: parseInt(args[2]), IfPromoUser: 0 });
