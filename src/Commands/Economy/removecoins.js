...
module.exports = {
	name:"removecoins",
    category:"economy",
	description: "Админская команда убирание коинов",

	async run (bot,message,args) {
		if(message.member.hasPermission('ADMINISTRATOR') || message.author.id === "691029995656839229") {
        if(!args[0]) {
            await message.delete();
            return await message.channel.send(`\|\| Упомяните пользователя у которого хотите забрать !\|\|`)
                .then(msg => msg.delete({timeout:10000}));
        }
        const remove_coin_member = message.mentions.members.first();
        if(!remove_coin_member) {
            await message.delete();
        ...
        if(!args[1]) {
            await message.delete();
            return await message.channel.send(`\|\|Укажите количество семечек которое хотите забрать! \|\|`)
        ...
        User.findOne({userID: remove_coin_member.id}, async(err, data) => {
            if(!data) {
                await message.delete();
            ...
            let a = new Discord.MessageEmbed()
                .setDescription(`Вы успешно забрали у <@${remove_coin_member.user.id}> семечки в количестве \`${args[1]}\``)
